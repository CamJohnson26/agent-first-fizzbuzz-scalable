# RCA 008: `corepack` unbundling in Node.js 24
...
- **2026-04-14**: Research confirmed that Node.js 24 unbundled Corepack, making it a separate package that must be installed via `npm`.
...
## Root Cause
1. **Node.js 24 Changes**: Starting with Node.js 24, Corepack is no longer included in the default distribution. Our project upgraded to Node.js 24.14.1 without accounting for this unbundling.
2. **Docker Image Minimalist Design**: The `node:24.14.1-slim` image provides a minimal environment and does not include non-standard or unbundled tools like Corepack by default.
...
3. Updated `README.md` to provide correct instructions for enabling `pnpm` on Node.js 24+.
