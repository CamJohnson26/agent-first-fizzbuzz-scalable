...
4. **PR & Merge**: Push your branch to origin and open a Pull Request using `gh pr create --fill`. Ensure the build passes, then schedule an automatic merge via `gh pr merge --auto --merge`. NEVER merge locally into `main`.
